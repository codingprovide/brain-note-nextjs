...
export function Toolbar({ className }: ToolbarProps) {
  ...
  const { toolColor, setToolColor, stroke, setStroke } = useDropDownStore<DropDownState>(
    (state) => state
  );
  ...
  const handlePaintToolClick = (tool: string) => {
    ...
    setshowDropDown(tool === "Pen" || tool === "Highlight" ? true : false);
  };
  ...
  const penColor = ["rgb(0, 0, 0)", "rgb(239, 68, 68)", "rgb(59, 130, 246)", "rgb(168, 85, 247)" ];
  const highLightColor = ["rgba(250, 204, 21, 0.5)", "rgba(34, 197, 94, 0.5)", "rgba(251, 146, 60, 0.5)", "rgba(107, 114, 128, 0.5)" ];
  ...
  return (
    <div>
      ...
      {showPaintToolBar && (
        <div className="react-flow__panel top center flex flex-col">
          <div
            className={`bg-white rounded-lg border border-l border-r shadow-sm p-1.5 inline-flex items-center gap-1 ${className}`}
          >
            <div className="flex items-center gap-1">
              <ToolbarButton
            ...
            <div>
              ...
              >
                <div className="bg-white rounded-t-lg border-t border-l border-r shadow-sm p-1.5 inline-flex  items-center gap-1 ${className}">
                  <ToolbarButton
                    icon={<div className="h-5 w-5 rounded-full" style={{ backgroundColor: showColor[0]}} />}
                    label={showColor[0]}
                    active={toolColor === showColor[0]}
                    onClick={() => handleToolColorClick(showColor[0])}
                  />
                  <ToolbarButton
                    icon={<div className="h-5 w-5 rounded-full" style={{ backgroundColor: showColor[1]}} />}
                    label={showColor[1]}
                    active={toolColor === showColor[1]}
                    onClick={() => handleToolColorClick(showColor[1])}
                  />
                  <ToolbarButton
                    icon={<div className="h-5 w-5 rounded-full" style={{ backgroundColor: showColor[2]}} />}
                    label={showColor[2]}
                    active={toolColor === showColor[2]}
                    onClick={() => handleToolColorClick(showColor[2])}
                  />
                  <ToolbarButton
                    icon={<div className="h-5 w-5 rounded-full" style={{ backgroundColor: showColor[3]}} />}
                    label={showColor[3]}
